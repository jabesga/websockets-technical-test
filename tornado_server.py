...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def initialize(self):
        ...
        connection = pika.BlockingConnection(pika.ConnectionParameters('localhost')) # RabbitMQ
        self.channel = connection.channel()
        self.channel.queue_declare(queue='tasks')

    def open(self, *args):
        clients.append(self)
        self.callback = tornado.ioloop.PeriodicCallback(self.updateStatus, 120)
        self.callback.start()
        self.task_uuid = str(uuid.uuid1()); # Generate UUID
    ...
    def updateStatus(self):
        self.write_message({'status': self.r.get(self.task_uuid), 'uuid': self.task_uuid}) 
